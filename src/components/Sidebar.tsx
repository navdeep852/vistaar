...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-slate-900 dark:bg-slate-950 text-slate-300 dark:text-slate-400 flex flex-col h-screen fixed left-0 top-0 z-30 border-r border-slate-800 dark:border-slate-900 hidden lg:flex no-print transition-all duration-300`}>
      {/* Brand Header */}
      <div className="h-20 px-5 flex items-center justify-between border-b border-slate-800/80 dark:border-slate-800/50 bg-slate-950/40 dark:bg-slate-950/80">
        {isCollapsed ? (
          ...
          <div className="flex flex-col justify-center min-w-0 py-1">
            <img src={logoFullName} alt="VISTAAR" className="h-11 w-auto max-w-[200px] object-contain drop-shadow-md" />
            <span className="text-[9px] text-blue-400 font-bold tracking-widest block uppercase pl-0.5 mt-0.5">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                      isActive
                        ...
                        : 'text-slate-400 dark:text-slate-400 hover:text-white dark:hover:text-white hover:bg-slate-800/60 dark:hover:bg-slate-900/80'
                    }`}
                  >
      ...
      {/* Workspace & Footer */}
      <div className="p-4 border-t border-slate-800 dark:border-slate-900 bg-slate-950/40">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-slate-800/50 dark:bg-slate-900/50 border border-slate-700/50 dark:border-slate-800">
          <UserAvatar name={user?.name} avatarUrl={user?.avatarUrl} size="sm" />
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold text-slate-200 truncate">{user?.businessName || 'Main Workspace'}</p>
