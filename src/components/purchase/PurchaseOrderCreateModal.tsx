...
export const PurchaseOrderCreateModal: React.FC<PurchaseOrderCreateModalProps> = ({
    ...
    const initializeModal = async () => {
      ...
      try {
        ...
        if (initialPo) {
          ...
          const sup = (sups || []).find((s: any) => s.id === initialPo.supplierId);
          if (sup) {
            setSelectedSupplier(sup as any);
            setSupplierSearch(sup.name);
          }

          setItems((initialPo.items || []).map((it) => ({
  ...
  const handleUpdateLine = (index: number, key: keyof PurchaseOrderItem, value: any) => {
    const updated = [...items];
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden transition-colors">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/80">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Order inventory from suppliers. Creating a PO does NOT alter stock until goods are received.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="relative w-72">
                <input
                  type="text"
                  placeholder="Type product name/SKU to add line..."
                  value={productSearch}
                  onChange={(e) => {
                ...
                {showProductDropdown && filteredProducts.length > 0 && (
                  <div className="absolute right-0 top-full mt-1 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-30 max-h-56 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500 font-mono">SKU: {p.sku || 'N/A'}</p>
                          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Supplier Notes (Visible on Print)</label>
                <textarea
                  rows={2}
                  value={notes}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Internal Notes (Hidden from Supplier)</label>
                <textarea
                  rows={2}
                  value={internalNotes}
