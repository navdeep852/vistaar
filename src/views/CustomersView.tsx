...
export const CustomersView: React.FC<CustomersViewProps> = ({
  ...
  const handleSaveCustomer = async (e: React.FormEvent) => {
    ...
    if (whatsapp.trim() && !isValidIndianPhoneNumber(whatsapp, false)) {
      setWhatsappError('Enter a valid 10-digit mobile number starting with 6–9.');
      hasErr = true;
    } else {
      setWhatsappError('');
    }
    ...
    if (hasErr) {
      ...
      } else if (!isValidIndianPhoneNumber(phone, true)) {
        ...
        >
          <form onSubmit={handleSaveCustomer} className="space-y-6 max-w-4xl mx-auto bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-card">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <label htmlFor="customer-name" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase mb-1">
