...
export const ExpensesView: React.FC<ExpensesViewProps> = ({ onNavigateTab, activeTab }) => {
  ...
  const openEditModal = (exp: Expense) => {
    if (!exp) return;
    if (exp.sourceType === 'SALARY_PAYMENT') {
      showToast('This expense was generated by Payroll. Redirecting to Salary & Payroll to view or manage.', 'info');
      if (onNavigateTab) {
  ...
  const handleDeleteExpense = async (exp: Expense) => {
    if (!exp) return;
    if (exp.sourceType === 'SALARY_PAYMENT') {
      showToast('This expense was generated by Payroll. Manage or cancel it from the Salary & Payroll module to preserve financial ledger consistency.', 'info');
      if (onNavigateTab) {
        onNavigateTab('salary-payroll');
      }
      return;
                  ...
                  <tr key={e.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-3.5 whitespace-nowrap">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {e.sourceType === 'SALARY_PAYMENT' && (
                          <span
                            onClick={() => (onNavigateTab ? onNavigateTab('salary-payroll') : null)}
                            className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-md text-[9px] font-extrabold bg-indigo-100 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/80 cursor-pointer hover:bg-indigo-200 transition-colors"
                            title="Click to view in Salary & Payroll"
                          >
                            <span>Payroll</span>
                    ...
                    <td className="p-3.5 font-bold text-slate-900 dark:text-slate-100">
                      {e.category === 'Other' ? (
                        ...
                        <div>
                          <span>{e.expenseName || e.category || 'Expense'}</span>
                          {e.notes && <p className="text-[10px] font-normal text-slate-400 dark:text-slate-500 mt-0.5">{e.notes}</p>}
                        </div>
                      )}
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        {e.sourceType === 'SALARY_PAYMENT' ? (
                          <button
                            ...
                            title="View Salary Payment in Payroll"
                          >
