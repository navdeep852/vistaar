...
export const ConsultantLayout: React.FC<InvoiceTemplateProps> = ({
  invoice,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all">
      {/* Executive Consultant Header */}
      <div className="flex justify-between items-start pb-6 mb-6 border-b border-slate-300 break-inside-avoid">
        <div>
        ...
        <div className="text-right">
          <span className="px-3 py-1 bg-slate-900 text-white font-bold text-[10px] uppercase tracking-wider rounded-md">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
          <span className="text-[10px] font-bold text-slate-400 block uppercase">CLIENT ENGAGEMENT PAYEE</span>
        ...
        {showBankDetails && business.bankDetails?.accountNo && (
          <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 text-right">
            <span className="text-[10px] font-bold text-slate-400 block uppercase">BANK ADVISORY REMITTANCE</span>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-900 text-white font-bold uppercase text-[10px]">
              <th className="p-3">#</th>
              <th className="p-3">Advisory Deliverable / Milestone</th>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div>
          {showTerms && hasValue(invoice.terms) && (
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">ENGAGEMENT TERMS</span>
        ...
        <div className="p-4 rounded-xl bg-slate-900 text-white space-y-1.5 text-right">
          ...
          <div className="pt-2 border-t border-slate-700 flex justify-between items-center text-sm font-black text-slate-100">
