...
export const ModernCards: React.FC<InvoiceTemplateProps> = ({
  invoice,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 break-inside-avoid">
        <div className="p-4 rounded-2xl bg-slate-900 text-white shadow-sm flex items-center justify-between">
          <div>
            <LogoComponent branding={branding} businessName={business.businessName} fallbackStyle="badge" />
            <h1 className="text-base font-bold text-white mt-2">{business.businessName}</h1>
            <p className="text-xs text-slate-300">{business.phone} | {business.email}</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100 flex flex-col justify-between text-right">
          <div>
            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">INVOICE NUMBER</span>
            <h2 className="text-xl font-extrabold text-indigo-950 mt-0.5"># {invoice.invoiceNumber}</h2>
      ...
      {/* Client Card */}
      <div className="p-4 rounded-2xl border border-slate-200 bg-slate-50 mb-6 flex justify-between items-center text-xs break-inside-avoid">
        <div>
          <span className="text-[10px] font-bold text-slate-400 block uppercase">CLIENT INFO</span>
        ...
        <div className="text-right">
          ...
          <p className="text-xl font-bold text-rose-600">{formatCurrency(invoice.balanceAmount, currency)}</p>
        </div>
      </div>
      ...
      {/* Summary Cards */}
      <div className="grid grid-cols-2 gap-4 mb-6 text-xs break-inside-avoid">
        <div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-1.5 text-right">
          ...
          <div className="pt-2 border-t border-slate-700 flex justify-between items-center text-sm font-bold text-indigo-400">
