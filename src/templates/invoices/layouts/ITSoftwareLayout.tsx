...
export const ITSoftwareLayout: React.FC<InvoiceTemplateProps> = ({
  invoice,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all">
      {/* IT Software Modern Header */}
      <div className="flex justify-between items-start pb-6 mb-6 border-b border-indigo-200 break-inside-avoid">
        <div>
          <LogoComponent branding={branding} businessName={business.businessName} fallbackStyle="badge" />
          <h1 className="text-xl font-extrabold text-indigo-950 mt-2">{business.businessName}</h1>
          <p className="text-xs text-slate-500">{business.address}</p>
          <p className="text-xs text-slate-500">{business.phone} | {business.email}</p>
          {showGstin && business.gstin && <p className="text-xs font-bold text-indigo-700 mt-1">GSTIN: {business.gstin}</p>}
        </div>

        <div className="text-right">
          <span className="px-3 py-1 bg-indigo-50 border border-indigo-200 text-indigo-700 text-[10px] font-black uppercase rounded-full tracking-wider">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div className="p-4 rounded-xl bg-indigo-50/40 border border-indigo-100">
          <span className="text-[10px] font-bold text-indigo-700 uppercase block">SOFTWARE SUBSCRIBER</span>
        ...
        {showBankDetails && business.bankDetails?.accountNo && (
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-right">
            <span className="text-[10px] font-bold text-slate-400 uppercase block">WIRE PAYMENT DETAILS</span>
      ...
      {/* Line Items */}
      <div className="mb-6 rounded-xl border border-indigo-100 overflow-hidden shadow-xs">
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-indigo-950 text-indigo-200 font-bold uppercase text-[10px]">
              <th className="p-3">#</th>
              <th className="p-3">Subscription / License / Service</th>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div>
          {showTerms && hasValue(invoice.terms) && (
            <div className="p-4 rounded-xl bg-indigo-50/30 border border-indigo-100">
              <span className="text-[10px] font-bold text-indigo-700 uppercase block mb-1">SLA & SERVICE TERMS</span>
        ...
        <div className="p-4 rounded-xl bg-slate-900 text-white space-y-1.5 text-right shadow-md">
          ...
          <div className="pt-2 border-t border-slate-700 flex justify-between items-center text-sm font-black text-indigo-400">
