...
export const PortfolioLayout: React.FC<InvoiceTemplateProps> = ({
  invoice,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all">
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl mb-6 flex justify-between items-center break-inside-avoid">
        <div className="flex items-center gap-4">
          <LogoComponent branding={branding} businessName={business.businessName} fallbackStyle="badge" />
          <div>
            <h1 className="text-xl font-black text-white">{business.businessName}</h1>
        ...
        <div className="text-right">
          <span className="text-[10px] font-bold text-rose-400 uppercase tracking-widest block">PROJECT INVOICE</span>
          <h2 className="text-2xl font-black text-white mt-0.5"># {invoice.invoiceNumber}</h2>
          <div className="mt-1">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
          <span className="text-[10px] font-bold text-slate-400 block uppercase">CLIENT SPONSOR</span>
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 text-right space-y-1">
          <span className="text-[10px] font-bold text-slate-400 block uppercase">PROJECT TIMELINE</span>
          <p className="text-slate-600">Invoice Date: <strong>{formatDate(invoice.date)}</strong></p>
          <p className="text-slate-600">Payment Due: <strong>{formatDate(invoice.dueDate)}</strong></p>
          {invoice.referenceNumber && <p className="text-slate-600">Ref Code: <strong>{invoice.referenceNumber}</strong></p>}
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-100 text-slate-800 font-bold uppercase text-[10px]">
              <th className="p-3">#</th>
              <th className="p-3">Project Deliverable / Scope</th>
        ...
        <div>
          {showTerms && hasValue(invoice.terms) && (
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-[10px] font-bold text-slate-400 uppercase block mb-1">PROJECT TERMS</span>
