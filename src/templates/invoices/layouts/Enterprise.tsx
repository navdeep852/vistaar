...
export const Enterprise: React.FC<InvoiceTemplateProps> = ({
  invoice,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all border-t-8">
      ...
      <div className="flex justify-between items-start pt-2 pb-6 border-b border-slate-200 mb-6">
        ...
        <div className="text-right space-y-1">
          ...
          <table className="text-xs text-right ml-auto mt-2 text-slate-600">
            <tbody>
              <tr><td className="pr-3 text-slate-400">Issue Date:</td><td className="font-bold text-slate-900">{formatDate(invoice.date)}</td></tr>
              <tr><td className="pr-3 text-slate-400">Payment Due:</td><td className="font-bold text-slate-900">{formatDate(invoice.dueDate)}</td></tr>
              {invoice.referenceNumber && <tr><td className="pr-3 text-slate-400">PO / Ref #:</td><td className="font-bold text-slate-900">{invoice.referenceNumber}</td></tr>}
            </tbody>
          </table>
          <div className="mt-2">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 text-xs break-inside-avoid">
        ...
        <div className="p-3.5 rounded-xl border border-slate-200 bg-slate-50/50">
          <span className="text-[10px] font-bold uppercase text-slate-400 block mb-1">INVOICE METRICS</span>
          <p className="text-slate-600">Line Items: <strong className="text-slate-900">{invoice.items.length}</strong></p>
          <p className="text-slate-600">Currency: <strong className="text-slate-900">{currency} (INR)</strong></p>
          <p className="text-slate-600">Payment Due: <strong className="text-rose-700">{formatDate(invoice.dueDate)}</strong></p>
        </div>
      ...
      <div className="flex justify-between items-start gap-6 mb-8 break-inside-avoid">
        <div className="flex-1 space-y-3 text-xs">
          {showTerms && hasValue(invoice.terms) && (
            <div className="p-3 border border-slate-200 rounded-xl bg-slate-50">
              <span className="text-[10px] font-bold text-slate-400 block mb-1">ENTERPRISE PAYMENT TERMS</span>
        ...
        <div className="w-72 border border-slate-300 rounded-xl p-4 bg-slate-50 space-y-2 text-xs">
          <div className="flex justify-between text-slate-600"><span>Subtotal Excl. Tax:</span><span>{formatCurrency(invoice.subtotal, currency)}</span></div>
          {invoice.discountTotal > 0 && <div className="flex justify-between text-rose-600"><span>Total Discount:</span><span>- {formatCurrency(invoice.discountTotal, currency)}</span></div>}
          <div className="flex justify-between text-slate-600"><span>Applicable Tax:</span><span>{formatCurrency(invoice.taxTotal, currency)}</span></div>
      ...
      {/* Signatures */}
      <div className="pt-6 border-t border-slate-200 flex justify-between items-end break-inside-avoid text-xs">
        {showStamp && branding.stampUrl ? (
          <img src={branding.stampUrl} alt="Stamp" className="max-h-20 object-contain" />
        ) : <div />}

        {showSignature && (
          <div className="text-right space-y-1">
            {branding.signatureUrl && <img src={branding.signatureUrl} alt="Signature" className="max-h-14 object-contain ml-auto" />}
            <div className="w-52 border-t border-slate-900 ml-auto pt-1 text-center">
              <p className="font-bold text-slate-900">Finance & Accounts Director</p>
