...
export const FreelancerLayout: React.FC<QuotationTemplateProps> = ({
  quotation,
  business,
  customer,
  branding,
    ...
    <div style={containerStyle} className="relative transition-all">
      ...
      <div className="flex justify-between items-start pb-6 mb-6 border-b border-slate-200">
        <div className="space-y-1">
          ...
          <h1 className="text-xl font-extrabold text-slate-900 mt-2">{business.businessName}</h1>
          <p className="text-xs text-slate-500">{business.ownerName ? `Contact: ${business.ownerName}` : business.email}</p>
          <p className="text-xs text-slate-500">Ph: {business.phone}</p>
        </div>

        <div className="text-right">
          <span className="px-3 py-1 bg-emerald-50 text-emerald-700 font-bold text-[10px] uppercase rounded-full tracking-wider">
            PROJECT QUOTE
          </span>
      ...
      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 mb-6 flex justify-between items-center text-xs break-inside-avoid">
        <div>
          <span className="text-[9px] font-bold uppercase text-slate-400 block mb-1">HI</span>
          <h3 className="text-sm font-bold text-slate-900">{customer?.name}</h3>
          <p className="text-slate-600">{customer?.address}</p>
        </div>
        <div className="text-right">
          <span className="text-[9px] font-bold uppercase text-slate-400 block mb-1">TOTAL COST</span>
          <p className="text-base font-bold text-slate-900" style={{ color: primaryColor }}>{formatCurrency(quotation.grandTotal, currency)}</p>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-100 text-slate-700 font-bold text-[10px] uppercase">
              <th className="p-3">Task / Service</th>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6 text-xs break-inside-avoid">
        <div>
          {showTerms && hasValue(quotation.terms) && (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-[9px] font-bold text-slate-400 uppercase block mb-1">PAYMENT & TERMS</span>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
          ...
          <div className="pt-2 border-t border-slate-300 flex justify-between items-center text-sm font-bold" style={{ color: primaryColor }}>
        ...
        {showSignature && (
          <div className="text-right">
            ...
            <div className="w-40 border-t border-slate-300 ml-auto pt-1 font-bold text-slate-800">
              {business.ownerName || 'Freelance Specialist'}
            </div>
          </div>
        )}
      </div>
