...
export const SignatureLayout: React.FC<QuotationTemplateProps> = ({
  quotation,
  business,
  customer,
  branding,
  ...
  return (
    <div style={containerStyle} className="relative transition-all">
      {/* Signature Verification Banner Header */}
      <div className="flex justify-between items-center pb-6 mb-6 border-b-2 border-slate-900">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900 uppercase">{business.businessName}</h1>
        ...
        <div className="text-right">
          <span className="px-3 py-1 bg-slate-900 text-white font-black text-[10px] uppercase rounded-full tracking-wider">
            VERIFIED QUOTATION
          </span>
      ...
      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 mb-6 flex justify-between items-center break-inside-avoid">
        <div>
          <span className="text-[9px] font-extrabold uppercase text-slate-400">CLIENT RECIPIENT</span>
        ...
        <div className="text-right">
          <span className="text-[9px] font-extrabold uppercase text-slate-400">VALUATION</span>
          <p className="text-lg font-black text-slate-900">{formatCurrency(quotation.grandTotal, currency)}</p>
        </div>
      </div>
      ...
      {/* Prominent Verification Signatures Box */}
      <div className="p-4 rounded-xl border-2 border-slate-900 bg-slate-50 flex justify-between items-end text-xs break-inside-avoid">
        {showStamp && branding.stampUrl ? (
          <img src={branding.stampUrl} alt="Stamp" className="max-h-16 object-contain" />
        ) : (
          <div>
            <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">OFFICIAL VERIFICATION</span>
            <p className="text-xs font-bold text-slate-800">Digitally Verified Document</p>
          </div>
        )}
        ...
        {showSignature && (
          <div className="text-right">
            ...
            <div className="w-48 border-t-2 border-slate-900 ml-auto pt-1 font-black text-slate-900">
